...
public struct ScrollsWithKeyboard<Content: View>: View {
	...
	private func setupKeyboardObservers(proxy: ScrollViewProxy) {
		let showPub = NotificationCenter.default.publisher(for: UIResponder.keyboardWillShowNotification)
		let hidePub = NotificationCenter.default.publisher(for: UIResponder.keyboardWillHideNotification)
		showPub.sink { handleKeyboard($0, isShowing: true)  }.store(in: &cancellables)
		hidePub.sink { handleKeyboard($0, isShowing: false) }.store(in: &cancellables)
	}
